...
update a cache with the return value of a method, you are com.msr.better off implementing an
...
taken by Spring is simpler and a com.msr.better match to its proxy-based, execution only
...
respectively. For example, the previous pointcut may be com.msr.better written as:
